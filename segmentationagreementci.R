...
compute_segmentation_agreement_ci <- function(alpha = 0.05, 
    ...
    # Expected counts
    expected_observer_pairs <- n_observer
    expected_device_pairs_option1 <- n_observer * (n_observer - 1) / 2
    expected_device_pairs_option2 <- n_observer * (n_observer - 1)
    ...
    if (n_observer_cols != expected_observer_pairs) {
        stop(sprintf("Expected %d observer pair columns for %d observers, but found %d columns matching 'DSC.observer.pair.*'", 
    ...
    if (n_device_cols != expected_device_pairs_option1 && n_device_cols != expected_device_pairs_option2) {
        stop(sprintf("Expected %d or %d device-observer pair columns for %d observers, but found %d columns matching 'DSC.device.observer.pair.*'", 
                    expected_device_pairs_option1, expected_device_pairs_option2, n_observer, n_device_cols))
    }
